...
### v1.0.3 重发补充
- 修复设置页打开即闪退（ListPreference 数组值被 AAPT2 编译成整数导致 NPE）
- 修复组件 mAh 数字消失（显示模式改为按实测宽度判断，<260dp 才紧凑）
- 容量表扩充到约 400 条主流机型；运行日志会打印命中的容量表键名
- 修复部分桌面把组件放成「高两格」（minHeight 60dp → 40dp，并开放竖向拖动修正）
